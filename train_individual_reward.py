# Based on: https://github.com/allenai/FineGrainedRLHF/blob/main/tasks/qa_feedback/training/train_finegrained.py
...
from fgrlhf import reward as R
...
class TextGenDataset(Dataset):
  ...
  def __init__(
      ...
      split,
      tokenizer,
      train_dataset='tasks/qa_feedback/data/train.json',
      dev_dataset='tasks/qa_feedback/data/dev.json',
      test_dataset='tasks/qa_feedback/data/test.json',
      accelerator=None,
      length_limit=None,
  ):
    super().__init__()
  ...
  def load_datasets(self):
    ...
    instances = instances[:len(instances) // self.n_card *
                          self.n_card]  # or Trainer will stuck
    return instances
...
def main(args: dict, log_info):
  ...
  eval_dataset = TextGenDataset(
      'dev',
      tokenizer,
      train_dataset=args['dataset']['dev'],
      accelerator=accelerator,
      length_limit=None,
  )
  ...
  ref_policy.model, ref_policy.linear = accelerator.prepare(
      ref_policy.model,
      ref_policy.linear,
  )
  policy = T5Policy(
  ...
  policy.model, policy.linear = accelerator.prepare(
      policy.model,
      policy.linear,
  )
  ...
  value = T5Value(
      ...
      model=policy.model
      if args['model']['value_model']['policy_value_sharing'] else None,
      tokenizer=tokenizer,
      accelerator=accelerator,
      freeze_model=False if args['model']['value_model']['policy_value_sharing']
      else args['model']['value_model']['freeze_value_model'],
  )
  ...
  reward = getattr(R, reward_config['model'])(
      tokenizer=tokenizer,
      model_ckpt=reward_config['ckpt'],
      **reward_config['args'],
      sep='</s>',
  )

  # TODO: test this to see if it places model on GPU
  if hasattr(reward, 'nf_reward_model'):
    reward.nf_reward_model = accelerator.prepare(reward.nf_reward_model)
  elif hasattr(reward, 'f_reward_model'):
    reward.f_reward_model = accelerator.prepare(reward.f_reward_model)
  elif hasattr(reward, 'model'):
  ...
  total_steps = ceil_div(
      args['train']['total_episodes'],
      args['train']['sampling_batch_size_per_card'] *
      accelerator.num_processes * args['env']['train_num_samples_per_input'],
  )
  ...
  scheduler = transformers.get_scheduler(
      ...
      args['train']['n_ppo_epoch_per_rollout'] * accelerator.num_processes,
  )
